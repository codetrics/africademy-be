...
- Prefer built-ins: `#[Route]` attributes, `#[AsCommand]`, `#[IsGranted]`, autowiring for DI, `Request`/`Response` objects, `ValidatorInterface`, form component for admin forms, `EventSubscriberInterface`/`#[AsEventListener]` for cross-cutting concerns.
...
- Typed constants for enumerations.
