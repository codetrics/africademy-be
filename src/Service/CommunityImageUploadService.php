...
class CommunityImageUploadService
{
    private const string IMAGE_DIRECTORY = 'var/storage/community';
    ...
    /**
     * Stores the uploaded image under var/storage/community/ and returns its
     * project-relative path. Deletes the previously stored image, if any.
     */
    public function store(UploadedFile $file, ?string $previousPath = null): string
    {
        ...
        $absoluteDirectory = sprintf('%s/%s', $this->projectDir, self::IMAGE_DIRECTORY);
        ...
        return sprintf('%s/%s', self::IMAGE_DIRECTORY, $fileName);
    }
